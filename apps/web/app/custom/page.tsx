...
export default function CustomizerPage() {
  ...
  const [activeLeftTab, setActiveLeftTab] = useState<
    "text" | "upload" | "shapes" | "roster" | "layers"
  >("text");
  ...
  const totalQuantity =
    Object.values(sizeQuantities).reduce((sum, q) => sum + Math.max(0, q), 0) ||
    1;
  ...
  const GARMENT_TYPES: Array<{
  ...
  }> = [
    {
      ...
      basePrice: PRICES.tshirt,
      desc: "Classic crew neck soft cotton tee",
    },
    {
      id: "polo",
      name: "Polo Shirt",
      basePrice: PRICES.polo,
      desc: "Premium ribbed collar sporty polo",
    },
  ];
  ...
  const [frontMockupPreviewUrl, setFrontMockupPreviewUrl] =
    useState<string>("");
      ...
      currentViewRef.current === "front"
        ? frontStateRef.current ||
          (typeof window !== "undefined"
            ? sessionStorage.getItem("custom_front_state")
            : null)
        : backStateRef.current ||
          (typeof window !== "undefined"
            ? sessionStorage.getItem("custom_back_state")
            : null);
  ...
  useEffect(() => {
    ...
    const syncRealMockups = async () => {
      const frontMock = await generateRealViewMockup(
        garmentType,
        selectedColor.hex,
        "front",
        frontPreviewUrl,
      ...
      const backMock = await generateRealViewMockup(
        garmentType,
        selectedColor.hex,
        "back",
        backPreviewUrl,
  ...
  const handleAddText = () => {
    ...
    showToast(
      "Text Added",
      "Text object added to printable canvas.",
      "success",
    );
  };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        try {
          // Clear any existing design on the canvas to ensure only one image is added per side
          fc.clear();
          ...
          showToast(
            "Artwork Uploaded",
            "Graphic placed on garment workspace.",
            "success",
          );
        } catch (err) {
          ...
          showToast(
            "Upload Error",
            "Could not render artwork on canvas.",
            "error",
          );
        }
      };
  ...
  const handleAddShape = (
    shapeType: "rect" | "circle" | "triangle" | "star" | "shield",
  ) => {
    const fc = fabricCanvasRef.current;
    if (!fc) return;
    ...
    showToast(
      "Shape Added",
      `${shapeType.toUpperCase()} shape added to canvas.`,
      "info",
    );
  };
  ...
  const handleAddJerseyRoster = () => {
    ...
    showToast(
      "Roster Added",
      "Player name and number placed on garment.",
      "success",
    );
  };
  ...
  const deleteActiveObject = () => {
    ...
    // Clear raw artwork reference if there are no image objects left
    const objects = fc.getObjects();
    const hasImage = objects.some((obj) => obj.type === "image");
    if (!hasImage) {
      if (currentViewRef.current === "front") {
        rawFrontArtworkRef.current = "";
        setFrontPreviewUrl("");
      } else {
        rawBackArtworkRef.current = "";
        setBackPreviewUrl("");
      }
    }
    ...
    showToast(
      "Object Removed",
      "Selected element deleted from canvas.",
      "info",
    );
  };
  ...
  const clearCanvas = () => {
    ...
    if (currentViewRef.current === "front") {
      ...
      setFrontPreviewUrl("");
      if (typeof window !== "undefined")
        sessionStorage.removeItem("custom_front_state");
    } else {
      backStateRef.current = null;
      rawBackArtworkRef.current = "";
      setBackPreviewUrl("");
      if (typeof window !== "undefined")
        sessionStorage.removeItem("custom_back_state");
    }
  ...
  const handleAddToCart = async () => {
    ...
    if (!rawFrontArtworkRef.current && !rawBackArtworkRef.current) {
      showToast(
        "Design Required",
        "Please upload custom artwork for at least one side (front or back).",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const hasFrontPrint = !!rawFrontArtworkRef.current;
      const hasBackPrint = !!rawBackArtworkRef.current;

      // Cloudinary upload for transparent design PNGs
      let uploadedFrontCloud = "";
      let uploadedBackCloud = "";

      if (hasFrontPrint && finalFrontUrl) {
      ...
      const frontMockupData = await generateRealViewMockup(
        garmentType,
        selectedColor.hex,
        "front",
        hasFrontPrint ? finalFrontUrl : undefined,
      );
      const backMockupData = await generateRealViewMockup(
        garmentType,
        selectedColor.hex,
        "back",
        hasBackPrint ? finalBackUrl : undefined,
      );
      ...
      if (hasFrontPrint && rawFrontArtworkRef.current) {
        try {
          const res = await fetch(getApiUrl("/cloudinary/upload"), {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      if (hasBackPrint && rawBackArtworkRef.current) {
        try {
          const res = await fetch(getApiUrl("/cloudinary/upload"), {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      const customizationFee =
        (hasFrontPrint ? 150 : 0) + (hasBackPrint ? 150 : 0);
      const baseUnitPrice =
        (PRICES[garmentType] + customizationFee) * (1 - discountPercent / 100);
      ...
      showToast(
        "Added to Cart",
        "Custom apparel saved to shopping bag.",
        "success",
      );
      router.push("/cart");
    } catch (err) {
      ...
      showToast(
        "Submission Error",
        "Failed to process custom apparel order.",
        "error",
      );
    } finally {
      setUploading(false);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 py-3 border-b border-[#E8E2D6] bg-white rounded-2xl px-2 sm:px-5">
        ...
        <div className="flex items-center w-full sm:w-auto gap-1 sm:gap-2">
          <button
            ...
            className="p-2 border border-[#E8E2D6] rounded-md hover:bg-[#FDFAF6] text-[#7A736A] disabled:opacity-40 transition-colors"
            title="Undo"
          >
          ...
          <button
            onClick={clearCanvas}
            className="text-[0] sm:text-xs font-bold text-red-500 hover:text-red-600 px-1.5 sm:px-3 py-1.5 border border-red-200 rounded-md hover:bg-red-50 transition-colors flex items-center gap-0 sm:gap-1"
          >
          ...
          >
            {uploading ? (
              ...
              <>
                ...
                <span>Order Now (₹{totalPrice.toFixed(0)})</span>
              </>
            )}
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-4 bg-white border border-[#E8E2D6] rounded-2xl p-4 shadow-sm space-y-4">
          ...
          <div className="space-y-4 animate-fade-in duration-150">
            <label className="border-2 border-dashed border-[#E8E2D6] hover:border-[#F9A37E] rounded-2xl p-6 flex flex-col items-center justify-center gap-2 bg-[#FDFAF6] hover:bg-white cursor-pointer transition-all">
              ...
              <p className="text-[10px] text-[#A89B8A] text-center">
                High resolution transparent artwork recommended
              </p>
              <input
                type="file"
                accept="image/*"
        ...
        <div className="overflow-hidden lg:col-span-5 flex flex-col items-center justify-between border border-[#E8E2D6] bg-white rounded-2xl p-4 relative min-h-[540px] shadow-sm">
          ...
          <div className="w-full flex items-center justify-center sm:justify-between z-10 flex-wrap gap-2">
            ...
            <div className="flex items-center gap-1 bg-[#FDFAF6] border border-[#E8E2D6] rounded-full px-2 py-1">
              <button
                onClick={() => setStudioZoom(Math.max(0.8, studioZoom - 0.1))}
                className="p-1 text-[#7A736A] hover:text-[#4A453E]"
              >
                <ZoomOut className="w-3.5 h-3.5" />
              </button>
              <span className="text-[10px] font-black text-[#4A453E] w-8 text-center">
                {Math.round(studioZoom * 100)}%
              </span>
              <button
                onClick={() => setStudioZoom(Math.min(1.4, studioZoom + 0.1))}
                className="p-1 text-[#7A736A] hover:text-[#4A453E]"
              >
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-white border border-[#E8E2D6] rounded-2xl p-4 shadow-sm space-y-4">
            <h3 className="font-extrabold text-xs text-[#4A453E] border-b border-[#E8E2D6] pb-2">
            ...
            <div className="space-y-2">
              <label className="block text-[11px] font-bold text-[#7A736A]">
                Garment Color:{" "}
                <span className="text-[#4A453E] font-extrabold">
                  {selectedColor.name}
                </span>
              </label>
            ...
            <div className="space-y-2 pt-2 border-t border-[#E8E2D6]">
              ...
              <div className="grid grid-cols-5 gap-1 text-center">
                {SIZES.map((s) => (
                  <div
                    key={s}
                    className="border border-[#E8E2D6] rounded-xl p-1 bg-[#FDFAF6]"
                  >
                    <span className="block text-[10px] font-black text-[#4A453E] mb-1">
                      {s}
                    </span>
                    <input
                      type="number"
            ...
            {discountPercent > 0 ? (
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-2.5 text-center">
                <p className="text-xs font-extrabold text-emerald-700 flex items-center justify-center gap-1">
                  <Sparkles className="w-3.5 h-3.5" /> Bulk Savings Applied (
                  {discountPercent}% OFF)
                </p>
              </div>
            ) : (
              <p className="text-[10px] text-[#A89B8A] italic text-center">
                Order 6+ shirts to unlock 10% volume discount!
              </p>
            )}
          </div>
          ...
          {(frontPreviewUrl || backPreviewUrl) && (
            <div className="bg-white border border-[#E8E2D6] rounded-2xl p-4 shadow-sm space-y-3">
              <h4 className="font-extrabold text-xs text-[#4A453E]">
                2. Real-View Live Shirt Preview
              </h4>
              <div className="grid grid-cols-2 gap-2">
                <div className="border border-[#E8E2D6] rounded-xl p-2 bg-[#FDFAF6] flex flex-col items-center">
                  ...
                  <span className="text-[8px] font-black text-[#7A736A] mt-1 uppercase">
                    Front View
                  </span>
                </div>
                <div className="border border-[#E8E2D6] rounded-xl p-2 bg-[#FDFAF6] flex flex-col items-center">
          ...
          <div className="bg-[#FDFAF6] border border-[#E8E2D6] rounded-2xl p-4 shadow-sm space-y-4">
            <h4 className="font-extrabold text-xs text-[#4A453E] border-b border-[#E8E2D6] pb-1.5">
              3. All-Inclusive Summary
            </h4>
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between text-[#7A736A]">
                ...
                <span className="font-bold text-[#4A453E]">
                  ₹{(PRICES[garmentType] * totalQuantity).toFixed(2)}
                </span>
              </div>
              {customizationFee > 0 && (
                <div className="flex justify-between text-[#7A736A]">
                  <span>
                    Print Fee (
                    {hasFrontPrint && hasBackPrint ? "2 Sides" : "1 Side"})
                  </span>
                  <span className="font-bold text-[#4A453E]">
                    ₹{(customizationFee * totalQuantity).toFixed(2)}
                  </span>
                </div>
              )}
              {discountPercent > 0 && (
