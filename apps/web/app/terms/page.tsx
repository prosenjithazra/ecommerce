...
export default function TermsPage() {
  ...
  useEffect(() => {
    ...
    const fetchPolicy = async () => {
      try {
        let res = await fetch(getApiUrl('/policies/terms')).catch(() => null);
        if (!res || !res.ok) {
          res = await fetch('/api/policies/terms').catch(() => null);
        }
        if (res && res.ok) {
        ...
        console.error('[TermsPage] Failed to fetch policy:', err);
      } finally {
