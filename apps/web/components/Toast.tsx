  ...
  return (
    <div className="flex items-start gap-2 sm:gap-3 p-2.5 sm:p-4 bg-white border border-[#E8E2D6] shadow-xl rounded-lg max-w-[calc(100vw-2rem)] sm:max-w-sm w-full animate-fade-in-up pointer-events-auto overflow-hidden relative">
      ...
      <div className="flex-1 min-w-0">
        <h4 className="font-bold text-xs sm:text-sm text-[#4A453E] leading-snug">{toast.title}</h4>
        <p className="text-[11px] sm:text-xs text-[#7A736A] mt-0.5 leading-tight">{toast.message}</p>
      </div>
      <button
        onClick={() => onDismiss(toast.id)}
        className="text-[#A89B8A] hover:text-[#4A453E] transition-colors p-0.5 sm:p-1 flex-shrink-0"
...
export const ToastContainer: React.FC<{ toasts: ToastMessage[]; onDismiss: (id: string) => void }> = ({ toasts, onDismiss }) => (
  <div className="fixed bottom-4 sm:bottom-5 right-4 left-4 sm:left-auto z-[60] flex flex-col items-center sm:items-end gap-2 w-auto sm:w-full max-w-none sm:max-w-xs pointer-events-none">
