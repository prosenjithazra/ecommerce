...
export const SearchBar: React.FC<{ initialValue?: string }> = ({ initialValue = "" }) => {
  ...
  const [collapsed, setCollapsed] = useState(!!initialValue); // Collapse if already searched
  ...
  // Collapsed state — show compact chip + "Search again" button
  if (collapsed) {
    return (
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs text-[#7A736A]">Showing results for</span>
        <span className="text-xs font-extrabold text-[#4A453E] bg-[#FBD5C1]/40 px-3 py-1 rounded-full">
          &ldquo;{query}&rdquo;
        </span>
        <button
          onClick={() => {
